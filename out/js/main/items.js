...
App.select_item = function (mode, item) {
  ...
  App[`selected_${mode}_item`].element.scrollIntoView({
    block: "nearest"
  });
...
App.selected_valid = function (mode) {
  return App[`selected_${mode}_item`] && App.item_is_visible(App[`selected_${mode}_item`]);
};
...
App.do_item_filter = function (mode, select = true) {
  ...
  function matched(item) {
    ...
    if (filter_mode === "all") {
      ...
      match = item.audible && (check(title) || check(path));
    } else if (filter_mode === "pins") {
      match = item.pinned && (check(title) || check(path));
    } else if (filter_mode === "muted") {
      match = item.muted && (check(title) || check(path));
    } else if (filter_mode === "normal") {
      match = !item.audible && !item.pinned && (check(title) || check(path));
    }
    return match;
  }
...
App.process_item = function (mode, item) {
  ...
  if (mode === "tabs") {
    obj.index = item.index, obj.active = item.active;
...
App.show_item_window = /*#__PURE__*/function () {
  var _ref = _asyncToGenerator(function* (mode, repeat_filter = false) {
...
App.focus_or_open_item = /*#__PURE__*/function () {
  var _ref2 = _asyncToGenerator(function* (item) {
    let tabs = yield App.get_tabs();
    for (let tab of tabs) {
